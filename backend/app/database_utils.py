...
def connect_to_database(database_info: Dict[str, Any]):
    if database_info['database'] == 'postgres':
        ...
        return connect_to_bigquery(database_info['project'], database_info['cred_path'])
    else:
...
def create_tables(db_connection, database_info: Dict[str, Any], chain: str, relay_chain: str):
    if database_info['database'] == 'postgres':
        ...
        create_bigquery_tables(db_connection, database_info['dataset'], database_info['table'], database_info['project'])
    else:
        raise ValueError(f"Unsupported database type: {database_info['database']}")
...
def insert_block_data(database_info, db_connection, block_data, chain_name, relay_chain):
    if database_info['database'] == 'postgres':
        ...
        insert_block(db_connection, database_info['dataset'], database_info['table'], block_data)
...
def query_last_block(db_connection, database_info: Dict[str, Any], chain: str, relay_chain: str, block_num = None):
    ...
    if block_num is None:
        if database_info['database'] == 'bigquery':
            fetch_last_block_query = f"SELECT * FROM {database_info['dataset']}.{database_info['table']} ORDER BY number DESC LIMIT 1"
        else:
            fetch_last_block_query = f"SELECT * FROM blocks_{relay_chain}_{chain} ORDER BY number DESC LIMIT 1"
    else:
        if database_info['database'] == 'bigquery':
            fetch_last_block_query = f"SELECT * FROM {database_info['dataset']}.{database_info['table']} WHERE number='{block_num}' LIMIT 1"
...
def query_recent_blocks(db_connection, database_info: Dict[str, Any], chain: str, relay_chain: str, limit: int = 50):
    if database_info['database'] == 'postgres':
        from .postgres_utils import query
    elif database_info['database'] == 'mysql':
        from .mysql_utils import query_block_data as query
    ...
    if database_info['database'] == 'bigquery':
        fetch_last_block_query = f"SELECT * FROM {database_info['dataset']}.{database_info['table']} ORDER BY number DESC LIMIT {limit}"
    else:
        fetch_last_block_query = f"SELECT * FROM blocks_{relay_chain}_{chain} ORDER BY number DESC LIMIT {limit}"
